...
object Practice {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 加载数据到Hive
    val tbStockRdd = spark.sparkContext.textFile("E:\\study\\study\\MyWorkspace\\sparkAll\\sparkSql\\doc\\tbStock.txt")
    val tbStockDS = tbStockRdd.map(_.split(",")).map(attr => tbStock(attr(0), attr(1), attr(2))).toDS
    insertHive(spark, "tbStock", tbStockDS.toDF)

    val tbStockDetailRdd = spark.sparkContext.textFile("E:\\study\\study\\MyWorkspace\\sparkAll\\sparkSql\\doc\\tbStockDetail.txt")
    ...
    val tbDateRdd = spark.sparkContext.textFile("E:\\study\\study\\MyWorkspace\\sparkAll\\sparkSql\\doc\\tbDate.txt")
